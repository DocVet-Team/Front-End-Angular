...
adicionarEspecialidade(): void {
  // Adiciona uma nova especialidade ao array de especialidades
  (this.formulario.get('especialidades') as FormArray).push(this.formBuilder.control(''));
}

removerEspecialidade(index: number): void {
  // Remove a especialidade na posição especificada do array de especialidades
  (this.formulario.get('especialidades') as FormArray).removeAt(index);
}
}